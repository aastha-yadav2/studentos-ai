...
create table if not exists public.opportunities (
  ...
  category text not null default 'Software Engineering', description text not null,
  eligibility text[] not null default '{}', required_skills text[] not null default '{}',
  location text not null default 'Remote', stipend_prize text,
  source_url text not null, source_platform text not null default 'Curated',
  deadline timestamptz, status text not null default 'active' check (status in ('active','expired','archived')),
...
create table if not exists public.opportunity_matches (
  ...
  strengths text[] not null default '{}', missing_skills text[] not null default '{}',
  explanation text not null, computed_at timestamptz not null default now(),
  unique (user_id, opportunity_id)
);
create table if not exists public.user_opportunity_saved (
  user_id uuid references auth.users(id) on delete cascade,
  opportunity_id uuid references public.opportunities(id) on delete cascade,
  saved_at timestamptz not null default now(), primary key (user_id, opportunity_id)
);
create table if not exists public.user_opportunity_applications (
  ...
  status text not null default 'saved' check (status in (
    'saved','interested','applying','applied','interviewing','accepted','rejected','withdrawn','not_eligible','deadline_passed'
  )),
  applied_at timestamptz, notes text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique (user_id, opportunity_id)
);
