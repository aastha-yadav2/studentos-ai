...
export function useDashboardData() {
  const { user } = useAuth()
  const [data, setData] = useState({ tasks: [] as Task[], goals: [] as Goal[], deadlines: [] as Deadline[], runs: [] as PlannerRun[], recommendations: [] as Recommendation[], activity: [] as Activity[] })
  ...
  const load = useCallback(async () => {
    ...
    const [tasks, goals, deadlines, runs, recommendations, activity] = await Promise.all([
    ...
    const failed = [tasks, goals, deadlines, runs, recommendations, activity].find((result) => result.error)?.error
    if (failed) setError(failed.message); else setData({ tasks: tasks.data ?? [], goals: goals.data ?? [], deadlines: deadlines.data ?? [], runs: runs.data ?? [], recommendations: recommendations.data ?? [], activity: activity.data ?? [] })
    setLoading(false)
  }, [user])
  useEffect(() => {
